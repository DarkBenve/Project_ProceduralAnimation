...
namespace Script.ProceduralMovement.Spider
{
    ...
    {
        [SerializeField] ComponentSpiderMovement player3D;
        [SerializeField] PlayerPivot playerPivot;
        [SerializeField] FollowTarget followTarget;
        [SerializeField] ProceduralStepMoveV2 spider;
        ...
        void JumpEnd()
        {
            ...
            for (int i = 0; i < targets.Length; i++)
            {
                ...
                if (!((IList)targetsG1).Contains(targets[i]))
        ...
        bool LandingEstimation(out (Vector3 position, Quaternion rotation, float time) landingEstimation,
            bool gizmo = false)
        {
            ...
            for (float t = Time.fixedDeltaTime; t <= landingEstimationTime; t += Time.fixedDeltaTime)
            {
                ...
                CalculateNextPosRot(bodyPos, velocity, rot, landingTimeCrt, Time.fixedDeltaTime, out Vector3 nextPos,
                    out Quaternion nextRot, out Vector3 nextBodyPos);
        ...
        void CalculateNextPosRot(out Vector3 nextPos, out Quaternion nextRot) => CalculateNextPosRot(body.position,
            velocity, transform.rotation, landingEstimation.time, Time.deltaTime, out nextPos, out nextRot, out _);

        void CalculateNextPosRot(Vector3 bodyPos, Vector3 velocity, Quaternion rot, float landingTimeCrt,
            float deltaTime,
        ...
        bool CheckGroundCollision(Vector3 nextPos, out Vector3 hitPos, out Quaternion hitRot) =>
            CheckGroundCollision(transform.position, nextPos, transform.rotation, out hitPos, out hitRot);

        bool CheckGroundCollision(Vector3 pos, Vector3 nextPos, Quaternion rot, out Vector3 hitPos,
            out Quaternion hitRot, bool gizmo = false)
        {
        ...
        void UpdateTargets(float jumpTime)
        {
            if (jumpTime >= targetsJumpTime)
                ...
                for (int i = 0; i < targets.Length; i++)
                    targets[i].position = Vector3.Lerp(targetsJumpDown[i].position, targetsJumpUp[i].position,
                        targetsJumpCurve.Evaluate(jumpTime / targetsJumpTime));
        }
    }
}