...
namespace Script.ProceduralMovement.Spider
{
    public class ScanArmsAdvanced : Scan
    {
        ...
        [SerializeField] ComponentSpiderMovement componentSpiderMovement;
        ...
        List<(Vector3, Quaternion, float)> Scan(bool gizmo = false)
        {
            ...
            for (int i = 0; i < armCount; i++)
            {
                ...
                if (componentSpiderMovement && componentSpiderMovement.Velocity != Vector2.zero)
                {
                    float angleArmVelocity =
                        Vector3.Angle(componentSpiderMovement.Velocity3, Quaternion.Euler(0, angle, 0) * Vector3.forward);
                    float progress = Mathf.InverseLerp(90, 0, angleArmVelocity);
                    arcRadius += progress * componentSpiderMovement.Speed * armLenghtSpeedCoef;
                }
                ...
                for (int j = 0;
                     j < armPoints && PhysicsExtension.ArcCast(pos, rot, arcAngle, arcRadius, arcResolution, arcLayer,
                         out RaycastHit hit);
                     j++)
                {
