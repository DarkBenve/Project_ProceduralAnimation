...
{
    public class ProceduralStepMove : MonoBehaviour
    {
        ...
        private ComponentSpiderMovement _componentSpiderMovement;
        ...
        void Cache()
        {
            _componentSpiderMovement = GetComponent<ComponentSpiderMovement>();
            CacheOrbitsDists();
        }
        ...
        {
            if (_componentSpiderMovement.Velocity == Vector2.zero)
                return (orbit.position, orbit.rotation);

            Vector3 velocityForward = orbit.TransformVector(_componentSpiderMovement.Velocity3);
            ...
            float dist = _componentSpiderMovement.Speed * stepTime / 2;
        ...
        private IEnumerator DelayStep(int idx, float delay)
        {
            if (delay > 0)
        ...
        private IEnumerator Step(int idx)
        {
            ...
            while (t < stepTime)
            {
                ...
                target.position += stepHeight * _componentSpiderMovement.SpeedProgress * stepHeightCurve.Evaluate(progress) * leg.up;
